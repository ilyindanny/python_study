...
for x, y in dictionary.items():
    print(x, y)  # причем если словарь включен в кортеж как один из его элементов, то этот код тоже работает, только писать нужно for x, y in tuple[2].items(): то есть указывать индекс, под которым лежит словарь
