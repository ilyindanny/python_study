...
text_list = list(range(-2, 2))  # Тут range - один элемент, который выдает функции list свои внутренние значения.

# Или так:

s = [*range(5)]  # А тут range уже распакован и представляет из себя пять отдельных элементов, поэтому он в квадратных скобках, он как бы уже массив.
...
# Другими словами у словаря есть три метода: keys(), values() и items(), которые возвращают соответственно ключи, значения и пары ключ-значение.
