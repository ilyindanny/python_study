...
# Тут в основном классе как бы первая половина, а в подклассе вторая. в рещультате конда вызывается метод подкласса,
# его первая часть берется из класса, а вторая из подкласса:
...
class ShortInputExc(Exception):
	...
	def __init__(self, length, was_expected):
...
try:
	...
	print('ShortInputExc: Длина введённой строки -- {0}; ожидалось, как минимум, {1}'.format(ex.length, ex.was_expected))

#  После конструкции try возможны дополнительные действия в блоке finally: (независимо от исключения):
...
# Метод может возвращать более одного значения. При распаковке в переменные достаточно поставить звездочку,
# чтобы в одной переменной было присвоено все остальное:
