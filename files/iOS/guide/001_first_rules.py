...
# срез с шагом 3 в обратную сторону:
print(s[:1:-3])

# длина (это функция):
print(len(text))

# поиск, возвращает индекс первой буквы (это метод, потому что вызывается через точку):
text.find('a')

# поиск по началу строки (соответствие запроса началу строки:
text.startswith('a')
...
# форматирование:
# print(" ".join(map(str,s))) печатает массив ровнее (либо каждый элемент строки)

# Пример с join() и split() в одной строке:

print(''.join(input('enter text with "abc": ').split('abc')))

# Еще пример с join(), split() и if в одной строке:
...
b = int(input('введите число не нуль: '))
...
# функция map() позволяет присваивать переменным элементы литерала, разделяя по указанному разделителю 
...
# можно на ноль проверять выражения без приравнивания:
if a and b:
    print('ок')
...
# конструкция match (не работает в моем мобильном компиляторе)
# match a:
#     case 2:
#         print()
#     case _:
